...
void setSurfaceWH(uint32_t width, uint32_t height) {
    android::SurfaceComposerClient::Transaction{}
            .setLayer(gSurfaceControl, 1)
            .show(gSurfaceControl)
            .setPosition(gSurfaceControl, width, height)
            .setLayerStack(gSurfaceControl, ui::DEFAULT_LAYER_STACK)
            .apply();
}
...
MDisplayInfo getDisplayInfo() {
    ...
    getPhysicalDisplayId(displayId);
    ...
    // 获取手机的屏幕信息
    status_t err = SurfaceComposerClient::getDisplayState(displayToken, &state);
